class FomExamsController < ApplicationController
  ...
  def display_fom
    ...
    if current_user.admin_or_higher?
       ...
       @student_uid = "Coming Soon!"
       @comp1_quizzes, @comp_avg_quizzes,  @exam_headers = FomExam.exec_raw_sql(params[:user_id], params[:attach_id], params[:permission_group_id], params[:course_code] )
       block_code = @course_code.split("-").second
